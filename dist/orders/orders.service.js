...
let OrdersService = class OrdersService {
    ...
    async create(dto, userId, userRole) {
        ...
        if (dto.userId) {
            ...
            if (!customer) {
                throw new common_1.NotFoundException(`Customer dengan ID ${dto.userId} tidak ditemukan`);
            }
        ...
        for (const item of dto.items) {
            ...
            if (!menu) {
                throw new common_1.BadRequestException(`Menu ID ${item.menuId} tidak ditemukan`);
            }
            if (!menu.isAvailable) {
                throw new common_1.BadRequestException(`Menu "${menu.name}" tidak tersedia`);
            }
            if (menu.stock < item.quantity) {
                throw new common_1.BadRequestException(`Stok "${menu.name}" tidak mencukupi. Tersedia: ${menu.stock}`);
            }
        ...
        const safePaymentMethod = dto.paymentMethod.toUpperCase();
        const isPaidMap = {
            CASH: false,
            QRIS: true,
            EWALLETQ: true,
